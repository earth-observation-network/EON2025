...
We also need to dowload the boundaries of the national park from the following
link:
[National Park](https://cloud.hawk.de/index.php/s/2G4XCatn87S8FsX).
Place both files into the subfolder 'data' of this tutorial. 
...
target variable. By using the spectral variability as sampling criteria we 
also ensure, that we cover the spectral range with our sampling.

The imported S2 scene has nine spectral bands covering the electromagnetic 
spectrum from 490 nm (blue) to 1610 nm (swir_1). Due the the physical properties
of electromagnetics we can expect that wavelength which are similar carry
correlated information. To test this, we create a correation matrix of the nine
spectral bands. 
...
As expected, neighboring bands are highly correlated and thus carry redundant
information.
...
and vitality status of the National Park forests.
...
To extract pixel values for the sample location we need to define a plot design.
For this exercise we will simulate a circular fixed area plot with a radius of
13 m. 
```{r plotDesign}
...
One of the objectives of the stratified sampling with in the spectral clusters, 
was to ensure that we cover the spectral range of the target areas with our 
sample plots. By comparing the spectral distributions in the area and sample we 
can check if this was successful. 
