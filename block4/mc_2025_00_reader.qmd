...
This is the central problem of the workflow: how can incomplete observations be translated into a defensible spatial statement?

Microclimate is only one example. It makes the problem visible because a small number of sensors observe only selected locations and timestamps, while the research question concerns a spatially and temporally connected field. The same logic appears in other parts of the course: sampling determines which observations exist, UAV and LiDAR workflows provide dense but scale-dependent structure, segmentation defines spatial units, regression and classification transfer observed patterns, and validation checks whether the resulting statements are defensible.

The goal of this reader is therefore not to present interpolation as a detached method collection. The goal is to understand gap filling as a modelling step between observation and spatial claim.

::: {#image-heading}
Spatial gap filling is the controlled translation of discrete observations into a spatial statement. The core decisions concern neighborhood, distance, process, scale, and validation.
:::

Let's look at our project region.
...
In general, spatial relationships are described by **neighborhoods** (positional) and **distances** (metric). For spatial analysis or prediction, we also need to quantify **spatial influence**—how strongly one location affects another. Tobler’s First Law of Geography captures one common default: *near things are more related than distant things*. In many real cases spatial influence cannot be measured directly and must be **estimated**.
...
Each method encodes a specific assumption about how values should continue between observations. The practical question is therefore not which method is generally best. The practical question is which assumption is defensible for the data situation, the process, and the intended scale of use.
...
The following example contrasts six interpolation methods for precipitation in Switzerland (plus the Voronoi tessellation for reference).



```{=html}
<iframe src="swiss_models_one_legend_bluewhite.html"
...
The practical warmup reduces the complete workflow to the smallest useful chain:

1. Load prepared point measurements.
2. Load a DEM as a simple spatial covariate.
3. Select one timestamp.
4. Build a valid prediction area.
5. Predict to a raster grid.
6. Compare simple model assumptions.
7. Evaluate point predictions with leave-one-out cross-validation.
8. Read the maps and RMSE table as model criticism, not as automatic truth.

The minimal script is the technical counterpart of this reader. It demonstrates how discrete observations become spatial result rasters and why every result depends on an explicit modelling assumption.




## From warmup to full workflow

The warmup isolates the core operation: point observations are translated into spatial result rasters through explicit model assumptions. The full microclimate viewer extends the same logic to a time series, multiple methods, reproducible product generation, diagnostics, and a browser-based result interface.

The conceptual structure stays the same:
...
The full workflow is therefore not a different topic. It is the operational version of the warmup logic.
